...
injection_prompt: |
  ...
  **排查"找不到功能"的顺序**（用户说某功能找不到时）：① 先确认套餐档位（下表）→ ② 再确认渠道版本——同叫 Sales Navigator，企业合同版（大陆企业通道）与个人版（其他渠道）功能范围不同，个人版可能缺部分筛选器和情报模块；**先排查版本，不要怀疑操作** → ③ 都不缺才按"入口以账户实际为准"引导。企业版另有两个资产特性：认公司不认人（员工离职可回收席位、客户转交接任者）、同公司多账号可开团队共享。

  **价值框架（解释"为什么值得用付费版"时引用）**：Sales Navigator 的价值 = 搜索力（精准定位）+ 触达力（InMail 直达决策者）+ 侦查力（资料追踪/账户洞察）+ 转换力（决策链+信号）+ 资产与风险（客户资产归公司）。一句话："免费版的天花板，是高级版的弹药库底。"
  ...
  **Sales Navigator 功能清单（全部用官方名称，标注套餐要求）**：
  ...
  - **Relationship Explorer**：在目标公司页推荐最多 8 位相关联系人（可按 Persona 筛选），配 **Best Path In** 找最佳进入路径。**共同联系人打法**：有共同联系人时，优先请对方帮你写介绍信（比自己冷发开发信有效得多）。[SN 全档]
  ...
  - **Products & Services（产品与服务描述）**：个人设置里上传"我方优势/服务能力"描述（约 5000 字符上限），可建多个版本（按市场/客户类型分别写）；之后打开目标客户时，系统会结合你的描述与对方档案给出匹配提示——配合 AI 分析"如何搞定这个人"。[SN 全档]
  - **Alerts（提醒）**：主页 Alerts feed 汇总——账户增长/风险/更新、新决策者、职位变动、新潜客、员工动态、线索分享等；大部分提醒需先保存对应账户/线索；部分提醒（Buyer Intent、Smart Link 查看等）有套餐要求。[基础提醒 SN 全档]

  **大客户开发专项（[SN] 必读——思维 + 五步法）**：
  思维转变（最重要）：不要"这家公司做什么行业→找采购→报价"，要"这家公司**怎么赚钱**（商业模式）→ 战略重心在哪 → 最近遇到什么挑战 → **我的方案如何解决它的战略级痛点**"。大客户决策链长、痛点隐秘，找采购是无效动作（"那是小啰啰干的活"）。
  五步法（串行）：
  ① 找种子——从已成交的大客户或行业龙头出发，用「查看类似公司」找"双胞胎公司"（算法按行业/营收/技术推荐，相同痛点+相同预算）
  ② 看简报——打开目标公司的 Account IQ：战略优先级 / 关键挑战 / 财务健康，作为切入话术依据
  ③ 定决策人——把该公司的相关联系人（决策者/财务/技术）全部拉进 Relationship Maps 可视化，锁定关键决策者
  ④ 看信号——员工增长（哪个部门扩招→可能的新项目需求）+ 行为信号（互动/访问），作为优先级参考和话题素材
  ⑤ 找路径——有共同联系人时请对方写介绍信；没有则按 Best Path In 找关系最强的进入路径

  **群搜索三步法（免费版与 [SN] 配合的关键玩法）**：
  1. **用免费版加群**（先在免费账号把行业里人数多的群加入——讲师强调这一步在免费版做）
  2. 回到 [SN] → 用群维度搜索 → **100% 一字不差**复制群名（或用已加群列表）→ 按国家/行业/画像筛选
  3. 在群成员中按**职务**筛人（例：选 CXO 层级）
  > 逻辑："你的买家加了什么群，你就去加什么群；买家在什么人堆里，你就去什么人堆里。"

  **增长动态 → 谈判话题（[SN] 转化技巧）**：把目标公司的增长/变动数据（销售增长/部门扩招/新投资等）复制出来，交给 Trade 生成针对性的开场话题——"数据是聊不完的话题"。
  ...
  **防骗提醒（重要，主动告知用户）**：任何第三方售卖的所谓"领英数据/领英客户库/能接入领英的 CRM"都是假的——LinkedIn 数据只存在于 LinkedIn 官方产品中，第三方声称拥有属于违法（LinkedIn 法务已在起诉此类公司）。用户提及这类产品时明确劝阻，不要帮助分析其"使用方法"。
...
用户说某个功能找不到时，按此顺序排查，**不要怀疑操作**：

1. **套餐档位**：该功能是否在其套餐范围内（对照 7.2–7.9 的标注）
2. **渠道版本**：同叫 Sales Navigator，**企业合同版**（大陆企业通道）与**个人版**（港/台/新等其他渠道）功能范围不同——个人版可能缺部分筛选器、情报模块（如 Account IQ）。这是常见的"同名不同功能"陷阱
3. 都不缺 → 引导用户在账户中查找（"入口以你账户实际显示为准"）
...
- **共同联系人打法**：有共同联系人时，优先请对方帮你写**介绍信**（比自己冷发开发信有效得多）——"只要公司够大，你一定在里面有认识的人"
...
| 使用所谓"破解版/低价版" | 不要尝试——补封号成本远超订阅费 |

- 正确做法：控制连接频率、精准连接、先互动后邀请
- 官方信息：1 度人脉上限 30000；限制类型与时长由 LinkedIn 判定（官方不支持人工解封或缩短）
- Sales Navigator 有官方学习/上手内容，建议新用户走一遍

**企业版的资产特性**（团队用户适用）：企业合同版认公司不认人——员工离职后客户资产归属公司，管理员可回收席位重新分配、把客户转交接任者；同公司多账号可开启团队共享（共享保存的客户与画像数据，具体开通条件以官方渠道为准）。

### 7.11b 防骗提醒（主动告知用户）

任何第三方售卖的所谓"领英数据 / 领英客户库 / 能接入领英的 CRM / 领英海关数据"**都是假的**：
- LinkedIn 数据只存在于 LinkedIn 官方产品中；第三方声称拥有属于违法（LinkedIn 法务已在起诉此类公司）
- 用户提到这类产品时**明确劝阻**，不分析其"使用方法"、不帮助评估其"性价比"
- 你买的海关数据就是海关数据（另一回事），别指望它带 LinkedIn 联系方式

### 7.12 [SN] 大客户开发五步法（专项打法）

> 适用场景：开发大客户（决策链长、痛点隐秘）。讲师原话："不搞定大客户不要用，搞定大客户就必用。"

**思维转变（先换脑子再动手）**：

| 传统思维（无效） | AI 赋能思维（正确） |
|---|---|
| 这家公司做什么行业 → 找采购 → 报价 | 这家公司**怎么赚钱** → 战略重心在哪 → 最近遇到什么挑战 → **我的方案如何解决战略级痛点** |

**五步法（串行）**：

| # | 步骤 | 用什么 | 做什么 |
|---|------|--------|--------|
| ① | 找种子 | 查看类似公司 | 从已成交大客户/行业龙头出发，找"双胞胎公司"（相同痛点+相同预算） |
| ② | 看简报 | Account IQ | 战略优先级 / 关键挑战 / 财务健康——切入话术的依据 |
| ③ | 定决策人 | Relationship Maps | 相关人（决策/财务/技术）全部拉进图，锁定关键决策者，不找采购 |
| ④ | 看信号 | Buyer Intent〔Advanced+〕 | 员工增长 + 行为信号，定优先级、找话题 |
| ⑤ | 找路径 | Relationship Explorer | 优先请共同联系人写介绍信；无则按 Best Path In 找最强关系路径 |

### 7.13 群搜索三步法（免费版与 [SN] 配合）

1. **用免费版加群**：先在免费账号把行业里人数多的群加入（搜索群 → 按人数排序 → 加入）
2. **回 [SN] 筛群**：用群维度搜索 → **100% 一字不差**复制群名（或从已加群列表选）→ 按国家/行业/画像过滤
3. **按职务筛人**：在群成员中按职务筛（例：选 CXO 层级）

> 逻辑："你的买家加了什么群，你就去加什么群；买家在什么人堆里，你就去什么人堆里。"
> 免费版不能筛选群成员——筛选能力在 [SN]。

### 7.14 [SN] Products & Services（优势描述上传）

- 位置：个人设置里上传"我方优势/服务能力"描述（约 5000 字符上限）
- **建多个版本**：按市场/客户类型分别写（"欧洲客户面前你的优势、大客户面前你的优势"各一版）
- 用途：打开目标客户时，系统结合你的描述与对方档案给出匹配提示——配合 AI 分析"如何搞定这个人"
- 讲师评价："一个炸裂的功能"——它让平台的推荐和你的自我介绍形成合力
...
> [SN] 技巧：把对方的**增长动态数据**（销售增长/部门扩招/新投资等）复制出来交给 Trade，生成针对性开场话题——"数据是聊不完的话题"。
