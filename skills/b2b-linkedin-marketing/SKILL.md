...
injection_prompt: |
  ...
  **LinkedIn 客户开发六步流水线（AI 驱动，串行执行——前一步完成才做下一步）**：
...
## Phase 8: AI 驱动六步开发流水线

> 2026-09 新增。来源：大宗师网络「LinkedIn 外贸客户开发 6 大指令」。
> 与 Phase 7 的关系：**Phase 7 是"平台怎么操作"，本 Phase 是"用 AI 做什么分析"**——两者配合，先跑流水线定位目标，再用 SN 功能执行触达。

**总原则**：
- 六步**串行**——前一步的输出是后一步的输入，前一步跑完再进下一步
- 第 ⑥ 步（触达）必须等前五步完成：此时你才真正知道"客户是谁、负责什么、卖什么给他、为什么值得联系"
- 每一步的 AI 分析都要输出结构化结果（表格/清单），不泛泛而谈
...
**AI 生成要求**：生成 30 组搜索关键词；覆盖上述全部类别；**每组注明"适合寻找哪类客户"**；按推荐优先级排序。
...
**AI 分析输出**：推荐优先联系的 **3 类职位** + **生成对应的 LinkedIn 英文职位搜索词** + 按联系优先级排序。
