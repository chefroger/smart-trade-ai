...
injection_prompt: |
  ...
  **排查"找不到功能"的顺序**（用户说某功能找不到时）：① 先确认套餐档位（下表）→ ② 再确认渠道版本——同叫 Sales Navigator，LinkedIn 在不同地区/渠道提供的版本功能范围可能不同；**先排查版本，不要怀疑操作** → ③ 都不缺才按"入口以账户实际为准"引导。
  ...
  **账号风控（全员适用）**：共享账号 = 永久封号（官方禁止）；来源不明的低价账号（共享账号等）有封号风险且不受官方保护，不建议购买；短时间大量加人 → 账号被限制发邀请（官方限制通常持续 1 周，累计多次可能永久封号）。正确做法：控制频率、精准连接；先互动后邀请。

  **防骗提醒**：任何声称拥有"领英数据/领英客户库/能接入领英的 CRM"的第三方产品都不可信——LinkedIn 不会向第三方提供会员数据，此类产品通常是骗局或违规手段，购买使用有风险。用户提及这类产品时明确劝阻，不要帮助分析其"使用方法"。
...
**企业合同版特性**（团队用户适用）：企业与 LinkedIn 签约的版本，账号与公司绑定——员工离职后管理员可回收席位重新分配；同公司多账号可开启团队共享（共享保存的客户与画像数据，具体范围以官方渠道说明为准）。

### 7.11b 防骗提醒

任何声称拥有"领英数据 / 领英客户库 / 能接入领英的 CRM / 领英海关数据"的第三方产品**都不可信**：
- LinkedIn 不会向第三方提供会员数据；此类产品通常是骗局或违规手段（爬虫、盗用账号等），购买使用有风险
- 用户提到这类产品时**明确劝阻**，不分析其"使用方法"、不帮助评估其"性价比"
- 你买的海关数据就是海关数据（另一回事），别指望它带 LinkedIn 联系方式

### 7.12 [SN] 大客户开发五步法（专项打法）

> 适用场景：开发大客户（决策链长、痛点隐秘）。
...
- **建多个版本**：按市场/客户类型分别写（欧洲客户一版、大客户一版）
