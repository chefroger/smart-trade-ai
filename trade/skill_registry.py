...
3. 内容发布（每周计划）：
   ...
   - 互动提问（20%）：引导评论，增加曝光
   - 客户背书（10%）：推荐信、好评截图
4. **LinkedIn 客户开发六步流水线（AI 驱动，串行——前一步完成才做下一步）**：
...
6. **会员能力分级（先确认用户等级，再给方案）**：区分免费会员与高级会员（Sales Navigator，SN）——不给免费用户推荐 SN 专属操作，每个 SN 功能都要给免费替代路径：
   - [SN] 四大金库（公司/人/销售素材/发信记录）= 一次工作量永久复用
   - [SN] 客户画像（只存职务，按市场分组）、保存搜索记录（新增客户自动追踪）、展会名录上传（≤1000家/表，CSV：公司名+URL 必填）
   - [SN] Account IQ（K people 相关人脉/决策者推荐/公司情报）、决策链图谱（标记支持者/反对者/锁定决策人）
   - [SN] 意向信号（员工增长=新需求 / 行为信号=高意愿）、Smart Link（谁看了/看了多久/转给了谁）、InMail 额度（50/月，上限150，回复返还）
   - [免费] 群必须免费版加（SN 加不了）；免费替代：手动固化搜索词 / Connection Request+Add Note / 主动跟进替代追踪 / 公开信息手工梳理决策链
7. **封号风控**：共享账号=永久封号；非正规渠道买个人版=100%封号；短时大量加人=警告3次以上永久封号
