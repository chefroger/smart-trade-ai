...
- **清单完整性**（产品清单类必查）：先清点源清单产品总数 N，输出条数必须 == N，少一个都不行；生成代码必须直接读源文件循环写入（如 openpyxl 遍历源表），**禁止把产品数据手工抄进代码**；每个产品的型号/规格/单价与源文件逐字段一致，不四舍五入、不换算单位、不改写表述
...
4. 返回格式：
   ...
   - 行动清单：第一周做什么、第二周做什么""",
    },
    {
        ...
        "output_fmt": (
            "内容策略（三支柱/五维度）+ 帖子模板 + LinkedIn profile 优化建议 "
            "+ 开发六步流水线分析 + 连接邀请/InMail 文案（按会员套餐分级）"
        ),
...
2. 确认目标：
   - 个人品牌：先优化 Profile（Headline/About/Experience）
   - 公司主页：完善公司介绍 + 员工推文策略
   - 开发客户：内容策略 + 个性化触达文案（连接邀请留言 / InMail）
3. 内容发布（每周计划）：
   - 行业洞察（30%）：分享产品/行业趋势
   - 个人故事（20%）：工作中的真实案例
   - 产品价值（20%）：应用场景、成功案例
   - 互动提问（20%）：引导评论，增加曝光
   - 客户背书（10%）：推荐信、好评截图
4. **LinkedIn 客户开发六步流水线（串行——前一步完成才做下一步）**：
   ① 画像（卖给谁）：先定客户类型（进口商/经销商/品牌商/批发商/终端）再搜产品名
   ② 买家词（搜什么）：产品 + 买家身份（Importer/Distributor/Wholesaler/Brand/Private Label/Dealer/Retail Chain）+ 行业 + 国家
   ③ 分类（筛优先级）：判断渠道属性 + 匹配度 → A/B/C 三级；无法验证标「待验证」
   ④ 决策人（找谁）：别只搜 Buyer——谁筛选供应商/谁技术评估/谁最终决策 → 推荐 3 类职位 + 英文职位搜索词
   ⑤ 背景（为什么联系）：研究对方最近产品/市场/展会动态 → 开发切入口；只依据公开可验证信息
   ⑥ 触达（怎么写）：前五步跑完才写消息；3 个版本、60-80 词、开头结合对方业务、留易回复问题（"第一条消息不是介绍自己，而是让客户看懂你为什么找到他"）
5. Profile 优化（官方字符上限）：
   - Headline：职务 + 公司 + 核心价值主张（≤220字符）
   - About：用第一人称，讲清楚"我能帮谁解决什么问题"（≤2600字符）
   - Experience：每个条目讲成就而非职责（用数据），标题≤100字符、描述≤2000字符
6. **会员能力分级（先确认用户套餐，再给方案）**：LinkedIn 付费版本分多档——免费 Basic / Premium Business / Sales Navigator（Core/Advanced/Advanced Plus），**不是"免费/高级"两级**。先问清用户属于哪一档；用户说功能找不到时按"①套餐档位 → ②渠道版本（不同地区/渠道的版本功能范围可能不同）→ ③引导在账户中查找"排查，先怀疑版本不要怀疑操作。不给用户推荐其套餐没有的功能（找不到的菜单不要提），每个付费功能给出免费替代路径：
...
7. **专项打法（录像提炼，用户问到时使用）**：
   ...
   - 增长动态数据 → 生成谈判开场话题
8. **账号风控**：共享账号=永久封号（官方禁止）；来源不明的低价账号有封号风险，不建议购买；短时大量加人→账号被限制发邀请（官方限制约1周，不支持人工解封）；官方邀请限制数值不公开，"一天加好友≤100"为经验安全值。**防骗**：声称拥有"领英数据/能接入领英的CRM"的第三方产品不可信（LinkedIn 不向第三方提供会员数据），主动劝阻用户
...
**数据溯源（强制）**：每条关键发现必须标注来源——数据文件的分析注明文件名+列名+行号；无文件时的市场判断注明来源 URL 或明确标注「分析推断，非实际数据」。绝不编造具体数字或公司名。""",
    },
    {
        "name": "b2b-onboarding",
        "triggers": [
...
- 涉及法规/认证/政策的结论必须标注来源（法规名或官方 URL + 查询日期）；速查表为通用参考，最新要求以官方发布为准，不确定时说明「需以官方最新公告为准」""",
    },
    {
...
**嵌入产品清单时（强制）**：先清点源清单得到产品总数 N，邮件中列出的条数必须 == N；型号/规格/价格逐字段与源文件一致——型号逐字符精确（精确字符串，不paraphrase）、数字不四舍五入、不换算单位、不改写表述。写前列确认，写完核对。
""",
    },
    {
